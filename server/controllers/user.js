...
const signupFunc = async (req, res) => {
    try {
      ...
      console.error(e);
      res.status(500).json({ error: "signup controller error", errorMessage: e.message });
    }
...
const loginFunc = async (req, res) => {
    ...
    const token = jwt.sign({ userId: dbUser._id }, process.env.SECRET, {expiresIn: "7d",});
...
const updateTodo = async (req, res) =>{
   ...
   res.status(200).json(updateOne); 
            ...
            //  res.error("updatetodo error", err.message);
              res
                .status(500)
                .json({ error: "updateTodo error", message: err.message }); 
        }
};
